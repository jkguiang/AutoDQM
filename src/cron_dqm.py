...
def handle_main(args):
        ...
        if new_database["files"]:
            with open("{0}/src/new_files.json".format(os.getcwd()), "w") as fhout:
                json.dump(new_database, fhout, sort_keys = True, indent = 4, separators=(',',':'))
        else: return search.make_response(args, len(new_database["files"]), False, None, None)

        return search.make_response(args, len(new_database["files"]), False, None, None)

if __name__=='__main__':

    args = json.loads(sys.argv[1])

    print(handle_main(args))
